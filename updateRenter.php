...
<html>
	...
	<body>
		...
		if (isset($_GET['RenterID'])) {
			echo "RenterID found: " . $_GET['RenterID'] . "<br>";

			$renterID = $_GET['RenterID'];
			
			echo "<a href='renter-details.php?RenterID=$renterID' class='btn custom-btn1'>Back</a>";
			...
			if ($result->num_rows > 0) {
				while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
					echo <<<_END
			<form action='updateRenter.php?RenterID=$renterID' method='post'>
				<pre>
					...
					<input type='hidden' name='RenterID' value='$row[RenterID]'>
		...
		_END;
				...
				if (isset($_POST['update'])) {
					...
					$query = "UPDATE renter SET Username='$username', FirstName='$firstname', LastName='$lastname', EmailAdd='$emailadd', PhoneNum='$phonenum' WHERE RenterID = $renterID ";

					$result_update = $conn->query($query);
					if (!$result_update) die($conn->error);

					// Redirect back to the referring page
					header("Location: renter-details.php?RenterID=" . $renterID);
					exit;
				}

				$result->close();
			} else {
				echo "No records found for the given RenterID.";
			}
		}
